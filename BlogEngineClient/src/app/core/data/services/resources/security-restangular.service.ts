...
export class SecurityRestangularService {
  ...
  constructor(
    ...
    this.baseUrl = basePath.securityApi;
  }
  ...
  one(path: string, id: string): SecurityRestangularService {
    const restangular = this.clone();
    restangular.baseUrl += (path ? '/' + path : '') + '/' + id;
    return restangular;
  }

  all(path: string): SecurityRestangularService {
    const restangular = this.clone();
  ...
  clone(): SecurityRestangularService {
    return new SecurityRestangularService(this._http, this.router, {
